...
export function ProjectsPage({ onNavigate }: ProjectsPageProps) {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      try {
        ...
        // Generate filter options from the valid data
        const uniqueCategories = [...new Set(validProjects.map(p => p.category))];
        const uniqueYears = [...new Set(validProjects.map(p => {
          try {
            return new Date(p.created_at || '2024-01-01').getFullYear().toString();
          } catch {
            return '2024';
          }
        }))];
        const uniqueStatuses = [...new Set(validProjects.map(p => {
          const status = p.status;
          return status === 'completed' ? 'Completed' : status === 'in-progress' ? 'In Progress' : 'Draft';
        }))];
        
        categories = ['All', ...uniqueCategories.filter(Boolean)];
        years = ['All', ...uniqueYears.filter(Boolean).sort().reverse()];
        statuses = ['All', ...uniqueStatuses.filter(Boolean)];
      } catch (error) {
  ...
  const filteredProjects = allProjects.filter(project => {
    ...
    if (!project || !project.title || !project.category) {
      return false;
    }
    ...
    try {
      ...
      try {
        projectYear = new Date(project.created_at || '2024-01-01').getFullYear().toString();
      } catch {
        projectYear = '2024';
      }
