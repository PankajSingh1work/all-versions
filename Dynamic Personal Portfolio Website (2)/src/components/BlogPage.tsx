...
export function BlogPage({ onNavigate }: BlogPageProps) {
  ...
  useEffect(() => {
    const loadBlogPosts = async () => {
      try {
        ...
        const validBlogPosts = (blogData || []).filter(post => 
          post && 
          typeof post === 'object' && 
          post.id && 
          post.title && 
  ...
  const filteredPosts = blogPosts
    .filter(post => {
      ...
      try {
        ...
        console.warn('Error filtering blog post:', error);
        return false;
      }
    })
    .sort((a, b) => {
      switch (sortBy) {
        ...
        case 'Most Commented':
          return ((b as any).comments || 0) - ((a as any).comments || 0);
        default: // Latest
  ...
  const totalLikes = blogPosts.reduce((sum, post) => sum + ((post as any).likes || 0), 0);
                ...
                >
                  <Card className="h-full overflow-hidden group hover:shadow-xl transition-all duration-300 hover:scale-105">
                    <div className="relative h-48 overflow-hidden">
                      <ImageWithFallback
                        src={post.featured_image || 'https://images.unsplash.com/photo-1649451844813-3130d6f42f8a?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=1080'}
                        alt={post.title}
                      ...
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        <div className="flex items-center space-x-4">
                          <div className="flex items-center space-x-1">
                            ...
                            <span>{formatDate(post.published_at || post.created_at || '2024-01-01')}</span>
                          </div>
                          <div className="flex items-center space-x-1">
                            ...
                            <div className="flex items-center justify-between text-xs text-muted-foreground">
                              <div className="flex items-center space-x-4">
                                <div className="flex items-center space-x-1">
                                  ...
                                  <span>{(post as any).author || 'John Doe'}</span>
                                </div>
                                <div className="flex items-center space-x-1">
                                  <Calendar className="h-3 w-3" />
                                  <span>{formatDate(post.published_at || post.created_at || '2024-01-01')}</span>
