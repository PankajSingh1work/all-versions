...
import { mockProjects, mockCertifications, mockBlogPosts, mockProfile } from './mock-data';
import type { Project, Certification, BlogPost, Profile } from './supabase';
...
export const mockApi = {
  ...
  certifications: {
    async getAll(): Promise<Certification[]> {
      ...
      return certifications.sort((a, b) => new Date(b.date_earned).getTime() - new Date(a.date_earned).getTime());
    },
    ...
    async create(certification: Omit<Certification, 'id' | 'created_at' | 'updated_at'>): Promise<Certification> {
      ...
      const newCertification: Certification = {
        ...certification,
        id: Date.now().toString(),
        slug: generateSlug(certification.title),
        created_at: now,
        updated_at: now
      };
    ...
    async update(id: string, updates: Partial<Certification>): Promise<Certification> {
      ...
      if (updates.title && updates.title !== certifications[certificationIndex].title) {
